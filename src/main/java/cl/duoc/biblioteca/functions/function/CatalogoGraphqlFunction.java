...
/**
 * Función Azure para manejar consultas GraphQL sobre el catálogo de libros y autores.
 * Permite consultar libros, autores, y realizar búsquedas por ID o por autor.
 */
public class CatalogoGraphqlFunction {

    // Se utiliza un ObjectMapper para parsear el body JSON de las solicitudes
    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();
    // Definición del esquema GraphQL para libros y autores
    private static final String SCHEMA = """
        type Query {
            ...
            libro(id: ID!): Libro
            autor(id: ID!): Autor
            librosPorAutor(idAutor: ID!): [Libro!]!
        }
        ...
        """;
    // Construcción del objeto GraphQL a partir del esquema y los dataFetcher
    private static final GraphQL GRAPHQL = buildGraphQL();

    // Función Azure para manejar consultas GraphQL sobre el catálogo de libros y autores
    @FunctionName("graphqlCatalogo")
    // Maneja solicitudes POST con consultas GraphQL en el endpoint /graphql/catalogo
    public HttpResponseMessage run(
            @HttpTrigger(
                    name = "req",
                    methods = {HttpMethod.POST},
                    authLevel = AuthorizationLevel.ANONYMOUS,
                    route = "graphql/catalogo")
        ...
        try {
            ...
            // Parseo del body como JSON para extraer query, operationName y variables
            ...
            // Construcción del ExecutionInput para GraphQL
            ...
            context.getLogger().log(Level.SEVERE, "Error en graphql catalogo", ex);
    ...
    private static GraphQL buildGraphQL() {
        ...
        RuntimeWiring wiring = RuntimeWiring.newRuntimeWiring()
                .type("Query", builder -> builder
                        .dataFetcher("libros", env -> OracleStore.getLibros())
                        .dataFetcher("autores", env -> OracleStore.getAutores())
                        .dataFetcher("libro", env -> OracleStore.getLibro(env.getArgument("id")))
                        .dataFetcher("autor", env -> OracleStore.getAutor(env.getArgument("id")))
                        .dataFetcher("librosPorAutor", env -> {
                            String idAutor = env.getArgument("idAutor");
                            return OracleStore.getLibros().stream()
                                    .filter(libro -> Objects.equals(libro.getIdAutor(), idAutor))
